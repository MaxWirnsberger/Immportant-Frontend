...
interface AreaDataFormValues {
  ...
  number_of_residential_units: number;
  cellar?: boolean | null;
...
const validationSchema = yup.object().shape({
  ...
  number_of_residential_units: yup
    .number()
    .transform((value, originalValue) =>
      String(originalValue).trim() === "" ? null : value
    )
    .typeError("Bitte geben Sie eine gültige Zahl ein")
    .min(1, "Wohnungsanzahl muss größer als 0 sein")
    .required("Wohnungsanzahl ist erforderlich"),
  plot_area: yup
    .number()
    .transform((value, originalValue) =>
      String(originalValue).trim() === "" ? null : value
...
export const TenementArea = forwardRef<FormHandle, AreaDataFormProps>(
      ...
      const fetchData = async () => {
        try {
          ...
          const formData: AreaDataFormValues = {
            ...
            number_of_residential_units: data.area?.number_of_residential_units,
            plot_area: data.area?.plot_area,
            garden_area: data.area?.garden_area || null,
            attic_area: data.area?.attic_area || null,
            ...
            cubature: data.area?.cubature || null,
            ...
            const nestedData = {
              area: {
                ...
                number_of_residential_units:
                  formData.number_of_residential_units,
                plot_area: formData.plot_area,
                attic_area: formData.attic_area,
          ...
          <div className={styles.formSection}>
            <div className={styles.formGroupRow}>
              ...
              <div className={styles.formGroup}>
                <Label className={styles.formLabel}>
                  <div>
                    Wohnungsanzahl<span className="text-red-500">*</span>
                    {errors.number_of_residential_units && (
                      <p className={styles.errorMessage}>
                        {errors.number_of_residential_units.message}
                      </p>
                    )}
                  </div>
                </Label>
                <Controller
                  ...
                  render={({ field }) => (
                    <Input
                      id="number_of_residential_units"
                      type="number"
                      {...field}
                      value={field.value ?? ""}
                    />
